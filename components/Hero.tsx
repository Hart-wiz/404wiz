...
export default function Hero() {
  return (
    <section id="home" className="px-6 lg:px-40 py-12">
      ...
      <div className="flex flex-col gap-8 flex-1">
        <div className="flex flex-col gap-4 text-center lg:text-left items-center lg:items-start">
          ...
          <h1 className="text-4xl lg:text-6xl font-black leading-tight tracking-[-0.033em] text-slate-900 dark:text-white">
            Building <span className="text-[#137fec]">robust</span> & <span className="text-[#00f2ff] neon-glow-cyan">scalable</span> web solutions
          </h1>
          <p className="text-slate-600 dark:text-slate-400 text-lg max-w-2xl leading-relaxed">
            Full stack software developer specializing in high-performance web applications, cloud architecture, and intuitive user experiences.
          </p>
        </div>
        <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
          <button className="flex min-w-[140px] cursor-pointer items-center justify-center rounded-lg h-12 px-6 bg-[#137fec] text-white text-base font-bold shadow-xl shadow-[#137fec]/30 hover:scale-[1.02] transition-transform">
            View Work
          </button>
          <button className="flex min-w-[140px] cursor-pointer items-center justify-center rounded-lg h-12 px-6 border border-slate-300 dark:border-[#283039] hover:bg-slate-100 dark:hover:bg-[#283039] text-slate-900 dark:text-white text-base font-bold transition-all">
            Contact Me
          </button>
        </div>
        {/* Quick Stats */}
        <div className="flex gap-8 pt-4 border-t border-slate-200 dark:border-[#283039] justify-center lg:justify-start">
          {[
            ...
            <div key={stat.label}>
              ...
              <p className="text-[10px] md:text-xs text-slate-500 uppercase font-semibold">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
